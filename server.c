...
int cl_count;
int pl_scores[CANTIDAD_MAXIMA_DE_JUGADORES]; //manque de temps pour shm
player players[CANTIDAD_MAXIMA_DE_JUGADORES];
	...
	while (running) {
		...
		if (game_in_progress) {
			if (end_of_turn && !sigempty) {
				...
				broadcast_light(JUEGUE, players, cl_count);
			}
			if (end_of_round) {
...
void add_client(int server_socket, struct sockaddr_in *cl_addr) {
	...
	if ((new_cl_socket = accept(server_socket, (struct sockaddr *)cl_addr, (socklen_t*) &cl_addr_length)) < 0) {
		...
		//players, cl_count
		if (game_in_progress || cl_count == CANTIDAD_MAXIMA_DE_JUGADORES) {
...
void add_player(int socket) {
	...
	enviar_mensaje_numerico(ESPERANDO_JUGADORES, COUNTDOWN, socket);
	if (cl_count == 1) {
...
void remove_player(player* players, int index, bool sockopen) {
	...
	memcpy(players[index].nickname, "\0", TAMANIO_DE_NOMBRE);
...
void deal_cards() {
	int cards_per_player = TAMANIO_DE_MAZO / cl_count;
	...
	for (player = 0; player < cl_count; player++) {
		...
		for (card = 0; card < cards_per_player; card++) {
			int random_card;
			do {
				random_card = rand_range(TAMANIO_DE_MAZO);
...
void clear_lobby() {
	broadcast_light(DESCONEXION, players, cl_count);
	game_in_progress = FALSE;
...
void receive_card(int socket, char** msg) {
	...
	static char cards[CANTIDAD_MAXIMA_DE_JUGADORES * 3];//:
	...
	if (received_cards_count == cl_count) {
		...
		enviar_mensaje(CARTAS_GANADAS, cards, players[highest_card_holder].socket);
		...
		if (sigempty) {
			broadcast_light(RONDA_TERMINADA, players, cl_count);
		}
	}
}
